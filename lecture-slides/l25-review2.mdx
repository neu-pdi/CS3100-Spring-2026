...
**Scope of Exam 2:** L12 (Domain Modeling) through L24 (Usability). Everything since Exam 1.
...
<ol style={{fontSize: '0.8em', textAlign: 'left', lineHeight: '1.9em'}}>
  ...
  <li><strong>Rapid-fire</strong> — requirements, GRASP, networks, teams/AI/OSS, usability (~12 min)</li>
  <li><strong>Exam logistics + Q&A</strong> (~2 min)</li>
</ol>

<p style={{fontSize: '0.82em', marginTop: '1.2em', fontStyle: 'italic', color: '#666'}}>
  Exam scope: L12–L24. One in-class exercise — bring your thinking caps.
</p>
...
<p style={{fontSize: '0.9em', marginTop: '0.5em', fontStyle: 'italic'}}>
  Answer at <strong>pollev.com/[your code]</strong>
</p>
...
A `CookbookManager` class calls into `Recipe` to get the list of ingredients, sums up the total calories, and returns the result. A colleague suggests moving the calorie calculation into `CookbookManager` instead. According to the Information Expert pattern, which is the better design?
...
An `EnergyPriceService` constructs a `new HttpClient()` inside its `shouldCharge()` method and calls the live grid pricing API directly. Which quality attribute does this most directly hurt?
...
A grading service receives a submission, starts processing, and then crashes. When it restarts, it processes the same submission again — the student gets double-graded. Which pattern would prevent this?
...
A team deploys their application as a monolith. They fix a one-line bug in the notification module and have to redeploy the entire system, which takes 20 minutes and takes the app offline. Which monolith characteristic explains this?
...
You're testing `OrderService.placeOrder()`. The method calls `EmailService.sendConfirmation()` as a side effect. You can't check the return value of `placeOrder()` to know if the email was sent. What should your test do?
...
## Teams, AI, OSS, and Usability (L22–24)
...
- **Five aspects:** Learnability, effectiveness, productivity, retainability, satisfiability
- **Nielsen's 10 heuristics:** know the concepts — system status visibility, error prevention, user control, consistency, recognition over recall, etc.
- **Mental models:** users have expectations; when software violates them, the user is wrong about the behavior but the *designer* is responsible for the gap
...
| **Scope** | L12–L24 (Domain Modeling through Usability) |
...
- Students asking "will X be on the exam?" — the answer is: if it was in L12–L24, it may appear

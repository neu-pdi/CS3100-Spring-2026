...
```mermaid
graph LR
    M[Model] -->|notifies| VM[ViewModel]
    VM -->|reads/updates| M
    VM ===|"two-way data binding"| V[View]
    V ===|"two-way data binding"| VM
    U[User] -->|interacts| V
...
<p style={{fontSize: '0.85em'}}>
The same "Activate Scene" handler — compare the Controller's job:
</p>
...
@FXML private void handleActivateScene() {
    ...
    if (scene != null) {
        model.activateScene(scene);
        // Must manually update EVERY widget
        updateDeviceList();
        brightnessSlider.setValue(
            model.getAverageBrightness());
...
Controller knows about widgets. 4 lines of manual sync.
...
@FXML private void handleActivateScene() {
    ...
    if (scene != null) {
        viewModel.activateScene(scene);
        // That's it. Binding handles the rest.
    }
}
```

Controller calls ViewModel. Binding updates all widgets automatically.
...
| | MVC | MVVM |
|---|---|---|
| **Coupling** | Controller → View widgets | ViewModel → nothing |
| **Sync** | Manual per widget | Automatic via binding |
| **Testable?** | Needs JavaFX runtime | Plain Java unit test |

</div>

<aside className="notes">
**This is the concrete version of the comparison.** The table alone is abstract — the code makes it visceral. MVC has 4 extra lines per action, and every one is a potential bug.

**When is MVC still fine?** For trivial UIs with 2-3 widgets. The MVVM overhead isn't worth it for a settings panel with one checkbox.

**When does MVVM pay for itself?** When you have multiple views of the same data, or when you need to test UI logic without a running app. GA1 features are in this category.
...
> **Transition:** This maps directly to a principle we learned earlier...
</aside>
...
## MVVM Fixes the Leaky Hexagon

<p style={{fontSize: '0.85em'}}>
In L29 we said MVC is hexagonal architecture for GUIs — but with a leak. The Controller straddles the boundary: decision logic + <code>@FXML</code> widgets in the same class. MVVM fixes that:
</p>
...
Controller has logic AND `@FXML` widgets. Hard to test.
...
graph LR
    ...
    style M2 fill:#c8f0dc,stroke:#4a9,color:#000
    style VM fill:#f0dcc8,stroke:#a94,color:#000
    style C2 fill:#d4c8f0,stroke:#94a,color:#000
    style V2 fill:#d4c8f0,stroke:#94a,color:#000
```

ViewModel is pure domain. Controller is thin infrastructure.
...
The ViewModel never imports <code>javafx.scene</code>. It's on the domain side of the hexagon — fully testable, no mocking needed.
</p>

<aside className="notes">
**This is the narrative payoff across L16 → L29 → L30:**
- L16: "Separate domain from infrastructure for testability"
- L29: "MVC does this, but the Controller leaks — it has logic AND widgets"
- L30: "MVVM fixes the leak — the ViewModel is pure domain, the Controller shrinks to a thin adapter"

**The ViewModel never imports `javafx.scene`.** It uses `javafx.beans` (properties) and `javafx.collections` (ObservableList), but never widgets. This is the equivalent of a domain class using an interface but never importing the infrastructure implementation.

**Point out on the diagram:** In MVC, the Controller is yellow (ambiguous — neither cleanly domain nor cleanly infrastructure). In MVVM, the ViewModel is orange (domain side) and the Controller is purple (infrastructure side, same as the View). The boundary is clean.
...
> **Transition:** ViewModel tests cover logic. But who tests that the bindings actually work?
</aside>

...
graph TD
    ...
    style SC fill:#d4c8f0,stroke:#94a,color:#000
```
...
<aside className="notes">
**The scene graph is the key concept.** Students need to understand that a GUI is a tree of objects — not a canvas you paint on. Every button, every label, every container is an object with properties, methods, and events. The framework traverses this tree to render the UI, handle focus order, and dispatch events.

**Analogy:** HTML is also a tree (the DOM). If students have web experience, this will click immediately. FXML is to JavaFX what HTML is to the browser — a declarative way to define the tree.

**Stage vs. Scene:** A Stage can swap its Scene (like a browser tab loading a different page). For GA1, students will typically have one Stage and one Scene.
...
<p style={{fontSize: '0.8em', marginTop: '0.3em', color: '#9370DB'}}>
<code>fx:controller</code> tells JavaFX which class to create. <code>fx:id</code> matches <code>@FXML</code> fields by name. <code>onAction="#method"</code> matches <code>@FXML</code> methods by name. The framework does the wiring — you just make the names match.
</p>
...
**Question 1: Event-driven programming**

A user clicks "Activate Scene" and then immediately drags the brightness slider. The `handleActivateScene()` handler takes 3 seconds to run. What happens to the slider drag?

- A. The slider moves smoothly — the framework handles both events in parallel
- B. The slider drag is queued and processed after `handleActivateScene()` finishes ✅
- C. The slider drag is silently dropped — the event is lost
- D. The app throws a ConcurrentModificationException

*Discussion:* The event loop is single-threaded. While `handleActivateScene()` is running, no other events can be processed — the slider drag is queued. The UI appears frozen for 3 seconds. This is why long-running handlers are a problem (and why we'll cover concurrency in L31-32). B and C are both plausible — discuss why it's B: the OS buffers input events in a queue. The event loop isn't ignoring them — it's busy and can't drain the queue yet. Once the handler returns, queued events are processed (the slider may "jump" to the final position). Intermediate move events may get coalesced, but the user's final intent is preserved.

---

**Question 2: MVC separation**
...
- A. The View (FXML file)
- B. The Controller
- C. The Model ✅
- D. The CSS stylesheet

*Discussion:* The Model contains all business logic. The Controller would call `model.turnOffAllLights()`, and the View would have a button wired to the Controller — but the logic itself (iterating devices, setting brightness) belongs in the Model where it's testable without the UI.

---

**Question 3: @FXML and the lifecycle**

Given this Controller, what happens when the FXML is loaded?
...
- A. The constructor sets the text; `initialize()` overwrites it with the same value
- B. The constructor crashes with a NullPointerException; `initialize()` would have worked ✅
- C. Both fail — you can only set text from FXML, not from Java
- D. The constructor works; `initialize()` is never called because the text is already set

*Discussion:* JavaFX creates the Controller (constructor runs) *before* injecting @FXML fields. In the constructor, `areaNameLabel` is still null → NPE. The `initialize()` method runs *after* injection, so the field is populated. This is the #1 lifecycle bug — use `initialize()`, not the constructor, for anything that touches @FXML fields.

---

**Question 4: Testability (forward-looking)**

You want to write a unit test that verifies: when the user activates the "Evening" scene, the device list shows "Ceiling Light: 30%". In our MVC architecture, what's the problem?

- A. The Model doesn't have an `activateScene` method — you'd need to add one
- B. The Controller's `handleActivateScene()` depends on `@FXML` widgets, so you can't call it without starting the full JavaFX runtime ✅
- C. Unit tests can't verify String values in lists — you'd need an E2E test
- D. There's no problem — you can test the Controller the same way you test any Java class
...
<div style={{display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75em', fontSize: '0.6em'}}>

<div>

<Img
  style={{maxWidth: '45%'}}
  src="/img/lectures/web/l29-dashboard-mockup.webp"
  prompt="Clean UI mockup (not pixel art) of a simple smart home area dashboard application window. 16:9 aspect ratio. The window title bar reads 'SceneItAll — Living Room'. Inside: at the top, a bold label 'Living Room'. Below that, a horizontal row with 'Brightness:' label, a slider set to about 70%, and a '70%' label next to it. Below that, a ListView showing three device statuses: 'Ceiling Light: 70%', 'Window Shades: Open', 'Floor Fan: Speed 2'. At the bottom, a horizontal row with a ComboBox dropdown showing 'Choose scene...' and a button labeled 'Activate'. Clean, minimal, standard platform UI widgets — looks like a real desktop application. Light gray background, standard UI chrome. Modern flat style, not pixel art."
  alt="Mockup of the SceneItAll area dashboard: Living Room header, brightness slider at 70%, device list showing three devices, and a scene selector dropdown with Activate button."
/>
...
<aside className="notes">
**This is the complete working example.** If you have time and a projector, run it live. If not, walk through the code explaining how each piece connects.

**Key things to point out:**
- The FXML and Controller are two halves of the same feature — the FXML defines structure, the Controller defines behavior
- `@FXML` annotations match `fx:id` values — this is the wiring
- `onAction="#handleActivateScene"` in FXML calls `handleActivateScene()` in the Controller
- The Controller calls `model.activateScene()` — it doesn't implement scene logic
- The ComboBox is populated from the Model — scene names come from `model.getSceneNames()`
- `updateDeviceList()` is the manual refresh — we'll automate this with binding next

> **Transition:** That manual refresh is annoying. What if the View could update automatically?
</aside>

...
<Slide>
  <Img
    ...
    prompt="Concept: 'User-Centered Design' (Program Design & Implementation 2). A pixel art educational illustration. On the left, a confident designer stands at a whiteboard covered in wireframes and checkmarks, smiling and giving a thumbs up — the design 'looks perfect.' On the right, behind a one-way observation mirror (clearly labeled 'Observation Room'), a real user sits at a laptop showing the same interface, looking completely lost — head tilted, question marks floating above, clicking the wrong buttons. The contrast is stark: the expert sees perfection, the user sees confusion. Between them, the one-way mirror reflects the designer's confidence back at them while the user's struggle is invisible. A clipboard on the wall shows 'Heuristic Evaluation: ALL PASS ✓'. Tagline at bottom: 'The Expert Approved It. The User Couldn't Use It.' Retro 8-bit pixel art style with teals, blues, purples, and warm accents."
    alt="Pixel art showing a designer giving thumbs up to wireframes that passed heuristic evaluation, while behind a one-way mirror a real user is completely lost using the same interface. Tagline: The Expert Approved It. The User Couldn't Use It."
  />
...
<strong>Connection to L24:</strong> Heuristic evaluation is valuable — but experts evaluating against principles are not the same as real users trying to accomplish real goals.
</p>
...
- Designer assumed "Reserve" was self-explanatory → Students searched for "Book" or "Get a room"
- Designer assumed 30-minute fixed slots → Students wanted custom time ranges like "3pm to 4:30pm"
...
<p style={{fontSize: '0.85em', marginTop: '1em', fontWeight: 'bold', color: '#9370DB'}}>
These aren't edge cases. These are the primary workflow. And no amount of expert review catches them — because <strong>experts aren't users</strong>.
</p>
...
"Wait, it says 3:00 to 3:30? <strong>We need at least an hour. How do I change that?</strong>"
</div>

</div>

<p style={{fontSize: '0.8em', marginTop: '0.5em', color: '#9370DB'}}>
The color coding reveals what to listen for: <span style={{color:'#4a9'}}>confidence</span>, <span style={{color:'#c90'}}>hesitation</span>, <span style={{color:'#4a4'}}>success</span>, <span style={{color:'#c44'}}>problems discovered</span>.
</p>
...
```mermaid
graph TD
    F["'Students can't book<br/>for longer than 30 min'"] --> A["Duration controls<br/>not visible?"]
    F --> B["Students don't understand<br/>the slot system?"]
    F --> C["Students expect to<br/>set their own times?"]
...
<p style={{fontSize: '0.85em', marginTop: '0.5em', color: '#9370DB'}}>
Same symptom, three different root causes, three different fixes. <strong>After implementing changes, test again</strong> — confirm the fix works without introducing new problems.
</p>
...
Interviews discover <strong>features</strong>. Prototypes discover <strong>requirements</strong> — including the organizing principles and workflows that make features usable.
</p>
...
<p style={{fontSize: '0.85em', marginTop: '0.3em', color: '#9370DB'}}>
...
<div style={{display: 'flex', flexDirection: 'column', gap: '0.8em', fontSize: '0.7em', marginTop: '0.5em'}}>

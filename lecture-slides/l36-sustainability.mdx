...
## First, Second, and Third-Order Effects Cascade Unpredictably
...
graph TD
    subgraph SceneItAll
        direction TB
        S1["1st: Hub uses power"] --> S2["2nd: Convenience<br/>increases energy use;<br/>data reveals occupancy"]
        S2 --> S3["3rd: Insurance and<br/>surveillance reshape<br/>around smart home data"]
    end
    subgraph Pawtograder
        direction TB
        P1["1st: Compute<br/>per submission"] --> P2["2nd: Unlimited submissions<br/>change study habits"]
        P2 --> P3["3rd: Auto-gradeable<br/>assignments narrow<br/>curriculum"]
    end
    subgraph LLMs["LLM Agents"]
        direction TB
        L1["1st: GPU inference<br/>per prompt"] --> L2["2nd: Developers write<br/>more code, explore<br/>more approaches"]
        L2 --> L3["3rd: Labor market<br/>restructures; codebases<br/>grow faster than teams"]
    end

    style S1 fill:#D6EAF8,color:#1a1a1a
    style S2 fill:#FDEBD0,color:#1a1a1a
    style S3 fill:#FADBD8,color:#1a1a1a
    style P1 fill:#D6EAF8,color:#1a1a1a
    style P2 fill:#FDEBD0,color:#1a1a1a
    style P3 fill:#FADBD8,color:#1a1a1a
    style L1 fill:#D6EAF8,color:#1a1a1a
    style L2 fill:#FDEBD0,color:#1a1a1a
...
<aside className="notes">
**Walk through one column.** Pawtograder is the most relatable: First order — each submission uses compute. Second order — unlimited submissions change how students learn (using the autograder as a debugger rather than thinking through code). Third order — if every course adopts autograding, assignments gravitate toward what's auto-gradeable, potentially narrowing what students learn.

**The question in the purple box is the design heuristic.** You can't predict all second- and third-order effects. But you can ask the question early and revisit it as the system scales.
...
<aside className="notes">
**The veil of ignorance is a design heuristic, not a rule.** It surfaces the trade-offs where the people benefiting and the people bearing risk are different groups. If you might be any stakeholder, you'd pay extra attention to the ones with the least power and most risk.
...
## The Veil Decides: Who Bears the Risk of Rate Limiting?

<div style={{fontSize: '0.72em'}}>

**Scenario:** Pawtograder implements IP-based rate limiting (max 10 submissions/minute per IP) to prevent autograder abuse.

Behind the veil, you might be:
...
| Stakeholder | Impact |
|------------|--------|
| Student on campus WiFi | Shared IP — hits limit during finals when everyone submits |
| Student at home | Stable IP — unaffected |
| Student using a VPN (required in some countries) | Shared VPN IP — locked out |
| TA testing grading scripts | Can request exemption |
| International student on university proxy | Proxy IP shared with hundreds — effectively blocked |

</div>

<p style={{fontSize: '0.85em', color: '#9370DB', marginTop: '0.3em'}}>
Does the veil of ignorance flag a fairness problem? If so, what's the fix?
</p>

<aside className="notes">
**This is the exercise that makes the veil of ignorance concrete.** Students must reason from behind the veil — if you might be ANY of these stakeholders, would you accept IP-based rate limiting?

**The answer is clearly no.** Students on campus WiFi and VPN users are disproportionately affected. The rate limiting decision benefits the infrastructure team (less abuse) but the risk falls on students who happen to share IPs — and those students are often the most vulnerable (international students, students in countries requiring VPNs).

**Possible fixes students might propose:**
- Rate limit by authenticated user, not IP
- Higher limits for shared IPs (detect by login count)
- Exempt known institutional IP ranges
- Rate limit by submission count per user, not per IP

**Key teaching point:** The "obvious" technical solution (IP-based limiting) embeds a value judgment about who matters. The veil of ignorance surfaces that judgment.
...
Every design decision encodes a value judgment — whether you think about it or not.
</p>

<aside className="notes">
**This is the intellectually honest slide.** We've been teaching students that good engineering means clear requirements, testable specifications, measurable quality attributes. Sustainability reveals the limits of that approach: some of the most important properties resist clean operationalization.
...
**Key message:** The goal of sustainability analysis isn't to resolve these tensions. It's to make them visible so decision-makers understand what they're trading off.

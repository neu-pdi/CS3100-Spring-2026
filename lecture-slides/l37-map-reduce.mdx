...
Stores data across thousands of machines. A **NameNode** manages metadata (which machines hold which chunks). **DataNodes** store the actual data.
...
**Terminology note:** The original Google papers and Hadoop used "master" and "slave" terminology. HDFS (the open-source implementation) has since renamed these to **NameNode** (metadata coordinator) and **DataNode** (storage worker). We'll use NameNode/DataNode for GFS components and **coordinator/worker** for MapReduce components throughout this lecture.

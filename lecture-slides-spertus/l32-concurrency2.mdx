...
<ol style={{fontSize: '0.75em', textAlign: 'left'}}>
  ...
  <li>Use <code>BackgroundTaskRunner</code> to divide work between GUI and bakckground threads</li>
  <li>Identify and avoid common asynchroonous programming pitfalls</li>
</ol>
...
<div style={{ fontSize: '.6em' }}>
https://github.com/cs3100-spertus-s26/concurrency-examples/blob/cmb-e/app/src/main/java/concurrency/CookMyBooks.java
</div>
...
## Pitfall 1: Timer Thread Mistakes

<div style={{fontSize: '0.8em'}}>

<div style={{backgroundColor: 'rgba(147,112,219,0.15)', padding: '0.8em', borderRadius: '8px', marginBottom: '0.8em'}}>

**GA1 Gotcha:** For the 5-second undo window (Library View) and 300ms debounce (Search & Filter), use JavaFX's `PauseTransition`. Alternatives like `ScheduledExecutorService` or `java.util.Timer` run callbacks on non-FX threads, requiring manual `Platform.runLater` wrapping.
...
**❌ Wrong: fires on wrong thread**
```java
ScheduledExecutorService scheduler =
    Executors.newSingleThreadScheduledExecutor();
scheduler.schedule(
    () -> commitDelete(),  // NOT on FX thread!
    5, TimeUnit.SECONDS);
```

</div>
<div style={{flex: '0 0 47%', backgroundColor: 'rgba(50,205,50,0.15)', padding: '0.8em', borderRadius: '8px'}}>

**✅ Correct: fires on FX thread**
```java
PauseTransition timer =
    new PauseTransition(Duration.ofSeconds(5));
timer.setOnFinished(e -> commitDelete());
timer.play();
```
...
<aside className="notes">
**Key point:** PauseTransition is a JavaFX animation primitive — it fires its handler on the FX Application Thread automatically, just like any other JavaFX event handler.

**Which features need this:**
- Library View: 5-second undo window
- Search & Filter: 300ms debounce delay
</aside>
...
```java
PauseTransition timer = new PauseTransition(Duration.ofSeconds(5));
...
## Poll: What should a Revert button handler do?

<PollSlide
  code={`
// User activates a scene:
PauseTransition revertTimer = new PauseTransition(Duration.ofSeconds(5));
revertTimer.setOnFinished(e -> hideRevertButton());
activateScene(scene);
showRevertButton();
revertTimer.play();

// User clicks Revert before timer expires.
// What should the Revert button handler do?
  `}
  choices={[
    "revertTimer.stop(); deactivateScene(scene);",
    "revertTimer.pause(); deactivateScene(scene);",
...
<aside className="notes">
**Answer:** revertTimer.stop() then deactivateScene(). stop() cancels the timer so hideRevertButton() never fires — we'll hide it ourselves as part of reverting. pause() is wrong — the timer could be resumed. deactivateScene() alone is wrong — the timer is still running and will call hideRevertButton() even though we've already handled the revert.
</aside>
...
<div style={{ fontSize: '.6em' }}>
https://github.com/cs3100-spertus-s26/concurrency-examples/blob/cmb-f/app/src/main/java/concurrency/CookMyBooks.java
</div>
...
<div style={{ fontSize: '.6em' }}>
https://github.com/cs3100-spertus-s26/concurrency-examples/blob/cmb-g/app/src/main/java/concurrency/CookMyBooks.java
</div>
...
Recall from L31: threads can cache variable values locally, leading to visibility problems — one thread's writes may not be seen by another thread.
```java
...
you need compound operations to be atomic (e.g., `count++` → use `AtomicInteger`)
...
<aside className="notes">
**Connection to L31:** Students saw the Java Memory Model and happens-before in L31. volatile is a lightweight synchronization mechanism — it guarantees visibility but not atomicity.

...
<div style={{display: 'flex', gap: '1.5rem', fontSize: '.8em', alignItems: 'flex-start'}}>
<div style={{flex: 1}}>
<Img src="/img/lectures/web/l29-javafx2.png" alt="Simple SceneItAlll GUI with dropdown menu for scene selection" />
...
Given a DashboardController where the constructor and `initialize()` both call `areaNameLabel.setText('Living Room')`,
what happens when the FXML is loaded?

<PollSlide
  username="espertus"
  choices={[
    "The constructor sets the text; initialize() overwrites it with the same value",
    "The constructor crashes with a NullPointerException; initialize() would have worked",
    "Both fail — you can only set text from FXML, not from Java",
    "The constructor works; initialize() is never called because the text is already set",
  ]}
/>
...
You want to unit test: when the user activates the 'Evening' scene, the device list shows 'Ceiling Light: 30%'.
Is there a problem in our MVC architecture?

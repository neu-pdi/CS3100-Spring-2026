...
**Structure (~28 slides):**
- Arc 1: From Usability to Accessibility and Inclusivity (~30 min) — running example (recipe import), spectrum, curb cut effect, GenderMag/SESMag with Fee vs. Dav walkthrough, invisible assumptions table, assistive technologies (vision, motor/hearing)
...
**Running example:** "Importing a recipe into CookYourBooks" — introduced early, revisited through different user lenses (blind, tremors, Dav/Fee, ESL) across all three arcs.
...
> **Transition:** Let's start where we left off in L24 and L27...
</aside>
...
- Step 2: "OCR extraction failed" → developer language. Fee blames the tool. Dav blames themselves.
- Step 3: Inline editing → risk tolerance. Fee experiments freely. Dav fears breaking things.
- Step 4: "Merge duplicates" → jargon. Fee ignores it. Dav can't proceed without understanding it.
...
| No undo vs. undo + reassuring text | User trusts that editing won't break things | Low self-efficacy users (Dav) | Add undo, show "You can always change this later" |
...
<div style={{backgroundColor: 'rgba(74,153,153,0.15)', padding: '0.8em', borderRadius: '8px', marginBottom: '1em'}}>
...
<div style={{backgroundColor: 'rgba(148,74,170,0.15)', padding: '0.8em', borderRadius: '8px', marginBottom: '1em'}}>
...
<div style={{backgroundColor: 'rgba(200,100,74,0.15)', padding: '0.8em', borderRadius: '8px', marginBottom: '1em'}}>
...
**Running example connection:** "OCR extraction failed" → Dav doesn't know what OCR means. "We couldn't read that image. Try a clearer photo." → everyone understands.

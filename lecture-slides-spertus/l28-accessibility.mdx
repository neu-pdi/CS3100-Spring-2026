...
**Structure (~28 slides):**
- Arc 1: From Usability to Accessibility and Inclusivity (~30 min) — running example (recipe import), spectrum, curb cut effect, GenderMag/SESMag with Fay vs. Dan walkthrough, invisible assumptions table, assistive technologies (vision, motor/hearing)
...
**Running example:** "Importing a recipe into CookYourBooks" — introduced early, revisited through different user lenses (blind, tremors, Dan/Fay, ESL) across all three arcs.
...
A modification made for the inclusion of an individual
...
3. Domino's didn't lose because accessibility is hard. They lost because they chose to fight rather than fix. The engineering cost of compliance was negligible compared to six years of legal fees.
...
**SESMag** extends this to socioeconomic status. People from different SES backgrounds interact with technology differently — not because of ability, but because of experience, access, education, and life circumstances. Key personas: "Fay" (high SES, experiments freely, blames the tool, comfortable with jargon) vs. "Dan" (low SES, sticks to familiar features, blames themselves, struggles with tech vocabulary).

**Examples to mention verbally:**
- "Session expired, please re-authenticate" — Fay logs back in. Dan on a shared computer thinks they're locked out.
- "Are you sure? This action cannot be undone" — Fay clicks through. Dan avoids the feature entirely.
- Auto-save to the cloud — great with reliable internet, silent data loss without it.

**Key insight for students:** Most of you are closer to Fay. You design for Fay by default. That's not malicious — it's invisible.
...
## Running Example: Importing a Recipe

<p style={{fontSize: '0.85em'}}>
Let's follow one user task through this entire lecture: <strong>importing a recipe into CookYourBooks.</strong>
</p>

<div className='fragment' style={{fontSize: '0.8em'}}>

The flow seems simple:

1. Click "Import" button
2. Paste a URL or upload a photo of a recipe
3. App extracts the recipe (title, ingredients, instructions)
4. Review the result, fix any errors
5. Save to your cookbook
...
<ul style={{fontSize: '0.8em'}}>
  ...
  <li>A user with <strong>tremors</strong> who can't drag-and-drop a photo</li>
  <li>A user on a <strong>shared library computer</strong> with 20 minutes left on their session</li>
  <li>A user whose <strong>first language isn't English</strong>, trying to parse "OCR extraction failed"</li>
</ul>
...
<aside className="notes">
**This is the throughline for the lecture.** We'll return to this import flow repeatedly:
...
## Same Feature, Different Experience: Importing a Recipe

<div style={{display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5em', fontSize: '0.75em'}}>

<div className='fragment' style={{backgroundColor: 'rgba(74,153,153,0.15)', padding: '0.8em', borderRadius: '8px'}}>

**Fay** (high self-efficacy, high risk tolerance)

1. Clicks "Import" — tries URL paste first
2. Sees "OCR extraction failed" — shrugs, tries a different URL
3. Notices the extracted recipe has a wrong ingredient — edits it inline
4. Hits "Save" without reading the confirmation dialog
5. **Total time: 90 seconds**

</div>

<div className='fragment' style={{backgroundColor: 'rgba(200,100,74,0.15)', padding: '0.8em', borderRadius: '8px'}}>

**Dan** (low self-efficacy, low risk tolerance)

1. Looks for "Import" — not sure which button it is (icon only, no label)
2. Sees "OCR extraction failed" — thinks *they* did something wrong. Tries again with the same URL.
3. Notices a wrong ingredient — afraid to edit it in case they break something
4. Reads every word of the confirmation dialog. Isn't sure what "merge duplicates" means. Closes the dialog to be safe.
5. **Gives up. Recipe not saved.**

</div>

</div>

<p className='fragment' style={{fontSize: '0.8em', marginTop: '0.8em', color: '#9370DB'}}>
...
<aside className="notes">
**This is the core GenderMag/SESMag insight made concrete.** Walk through each step and ask students: which design decision caused Dan's different experience?

- Step 1: Icon-only button → recognition vs. recall (H6). Fay recognizes it from other apps. Dan doesn't.
- Step 2: "OCR extraction failed" → developer language. Fay blames the tool. Dan blames themselves.
- Step 3: Inline editing → risk tolerance. Fay experiments freely. Dan fears breaking things.
- Step 4: "Merge duplicates" → jargon. Fay ignores it. Dan can't proceed without understanding it.

**The punchline:** Dan is not a less capable user. Dan is a user your software wasn't designed for. These are fixable design choices, not user deficits.

**Connection to GA0:** "Look at your personas. Are any of them closer to Dan than Fay? If not, you're only designing for one end of the spectrum."
...
<Img
  width={1100}
  src="/img/lectures/web/l28-import-comparison.webp"
  prompt="Clean educational illustration (not pixel art) showing a side-by-side comparison of two software interface mockups for a 'Recipe Import' screen, labeled 'Less Accessible' and 'More Accessible'. 16:9 aspect ratio. LEFT SIDE (labeled 'Less Accessible', red-tinted border): A recipe import dialog with these problems: (1) An unlabeled icon-only button at the top — just an arrow icon with no text, (2) A large drag-and-drop zone that says 'Drag photo here' with NO alternative upload button, (3) A red error message reading 'OCR extraction failed: invalid MIME type' in small monospace font, (4) A red/green progress indicator with no text labels — just colored circles, (5) Small, cramped 'Save' and 'Cancel' buttons with no undo option. RIGHT SIDE (labeled 'More Accessible', green-tinted border): The same recipe import dialog redesigned: (1) A clearly labeled button reading 'Import Recipe' with both an icon AND text, (2) A drag-and-drop zone WITH a visible 'Browse files' button underneath as an alternative, (3) A friendly error message in a clear box: 'We couldn't read that image. Try a clearer photo or paste the recipe URL instead.' with a helpful suggestion, (4) Status shown with BOTH colors AND text labels: a green checkmark with 'Imported successfully' text and a yellow triangle with 'Needs review' text, (5) Larger touch-friendly buttons with an 'Undo' option and reassuring text: 'You can always edit this later.' Both interfaces show the same basic layout and the same recipe (chocolate chip cookies) so the comparison is about design choices, not content. Clean, modern UI mockup style with clear annotations."
  alt="Side-by-side comparison: a less accessible recipe import interface (icon-only buttons, drag-only upload, technical error messages, color-only status) versus a more accessible version (labeled buttons, browse-files alternative, plain-language errors, text+color status, undo option)."
/>
</div>

<p style={{fontSize: '0.85em'}}>
Both interfaces import a recipe. Both have the same features. Look at the two versions and identify: <strong>what assumptions does the left version make about its users that the right version doesn't?</strong>
</p>
...
|-----------|----------------|-----|-----|
| Icon-only button vs. labeled button | User can see and recognize the icon | Blind users, low-vision users, unfamiliar users | Add a text label alongside the icon |
| Drag-only upload vs. drag + browse button | User can drag-and-drop | Users with tremors, keyboard users | Add a "Browse files" button |
| Technical error vs. plain language error | User can read developer jargon | Non-technical users, ESL speakers | "We couldn't read that image. Try a clearer photo." |
| Color-only status vs. color + text status | User can distinguish red from green | Colorblind users (~8% of men) | Add text labels or shapes alongside colors |
| No undo vs. undo + reassuring text | User trusts that editing won't break things | Low self-efficacy users (Dan) | Add undo, show "You can always change this later" |
...
<Img src="/img/lectures/web/l28-password-stars.png" alt="Login password field where typed characters are hidden as a row of asterisks"/>
<aside className="notes">
...
<div className='fragment' style={{backgroundColor: 'rgba(74,153,153,0.15)', padding: '0.8em', borderRadius: '8px', marginBottom: '1em'}}>
...
<div className='fragment' style={{backgroundColor: 'rgba(148,74,170,0.15)', padding: '0.8em', borderRadius: '8px', marginBottom: '1em'}}>
...
<div className='fragment' style={{backgroundColor: 'rgba(200,100,74,0.15)', padding: '0.8em', borderRadius: '8px', marginBottom: '1em'}}>
...
<div className='fragment' style={{backgroundColor: 'rgba(169,148,74,0.15)', padding: '0.8em', borderRadius: '8px'}}>
...
**Running example connection:** In CookYourBooks import flow — the imported recipe shows a photo. Without alt text, a screen reader user knows there's an image but not what it shows. Status indicators (success/failure) shown only in red/green exclude colorblind users.
...
**Running example connection:** The import flow requires drag-and-drop to upload a photo. A user with tremors can't do this. A keyboard user can't do this. Fix: provide a "Browse files" button as an alternative.
...
**Connection to SESMag:** Dan struggles with tech jargon and complex sentence structures. Fay can parse "exponential backoff." Most users are closer to Dan than Fay for unfamiliar software.
...
**Running example connection:** "OCR extraction failed" → Dan doesn't know what OCR means. "We couldn't read that image. Try a clearer photo." → everyone understands.
...
<p style={{fontSize: '0.85em'}}>
Back to our running example — the "Import" button in CookYourBooks:
</p>
...
What the user sees: a clickable "Import Recipe" button

What a screen reader announces: *"Import Recipe, button"*

What a keyboard user experiences: Tab highlights it, Enter activates it

What voice control hears: "Click Import Recipe" → works
...
What voice control hears: "Click Import Recipe" → *"No matching element found"*
...
**Focus management (rule 5)** is the one students are least likely to think about. Give a concrete example: "You click 'Import Recipe,' a dialog appears. You're a screen reader user. If focus doesn't move to the dialog, you don't know it appeared. You hear nothing. You press Tab and end up in the footer."
...
<PollSlide username='espertus'
  choices={["An accessibility expert", "Disabled people", "An automated checker", "Other"]}
/>
...
<div style={{backgroundColor: 'rgba(74,153,153,0.15)', padding: '1em', borderRadius: '8px', fontSize: '0.85em'}}>
...
<div>
<Img src="/img/lectures/poll-ev/qr-pollev-espertus.png" />
            <p style={{ fontSize: '0.8em', marginTop: '0.5em' }}>

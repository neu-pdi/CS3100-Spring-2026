...
Here's what AI coding tools change and what they don't:
...
| Context | AI helps | AI hinders |
|---------|---------|-----------|
| Unfamiliar API or language | Pattern matching, syntax lookup | — |
| Well-understood codebase, routine task | Boilerplate generation | — |
| Complex integration across modules | — | Lacks architectural context |
| Debugging subtle race conditions | — | Can't execute or reproduce |
| Greenfield prototype | Rapid iteration | — |
| Safety-critical production code | — | Removes human review layer ([L35](/lecture-notes/l35-safety-reliability)) |
...
| **Manual implementation** | No AI. You write every line. | Learning new concepts ("learning tax"), safety-critical code |
...
This mirrors how AI should be adopted in professional settings: build competence first, then introduce tools, then use them with accountability structures. The "restricted → structured → encouraged" progression is the same curriculum mapping approach that the [AI guidance for computing education](https://dl.acm.org/) recommends across entire programs.
...
But the failure rate is sobering. In December 2025, an AI coding agent at Amazon [autonomously deleted and recreated a live production environment](https://www.ruh.ai/blogs/amazon-kiro-ai-outage-ai-governance-failure), causing a 13-hour outage. [88% of organizations](https://www.helpnetsecurity.com/2026/03/03/enterprise-ai-agent-security-2026/) report confirmed or suspected AI agent security incidents. Every failure maps to a principle from this course: the Amazon incident is a violation of least privilege ([L4](/lecture-notes/l4-specs-contracts) — contracts and preconditions). Agents that hallucinate API calls are a testing problem ([L15](/lecture-notes/l15-testing)). Multi-agent systems are concurrent systems ([L31](/lecture-notes/l31-concurrency1)). An agent with unconstrained write access to production is a blast radius problem ([L35](/lecture-notes/l35-safety-reliability)). The organizations succeeding with AI agents are the ones applying classical software engineering discipline to a new class of component.

...
:::note
GitHub is a Ruby on Rails monolith serving over 100 million developers. As AI-generated traffic doubled their request volume, they optimized within the monolith — aggressive caching, database read replicas, and request-level performance budgets — rather than rewriting as microservices. Sometimes the right move is to optimize within your architecture's constraints rather than change the architecture. Who could have predicted that AI agents would put so much pressure on GitHub's infrastructure to crack the monolith? (Connects to [L19](/lecture-notes/l19-monoliths) and the GitHub availability discussion from L31.)
:::
